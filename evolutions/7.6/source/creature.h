...
public:
	...
	int64_t getExpForLv(const int& lv) const{ 
		int64_t x = lv;
	...
	void updateLookDirection(const Creature* creature = NULL);
	...
	void addEventThink();
	void addEventThinkAttacking();
	void stopEventThink();
	void stopEventThinkAttacking();
...
protected:
	...
	virtual int64_t getLostExperience() const { return 0; };
