...
{
	int32_t maxDamage = (attackSkill * attackValue)/20;
	double newDamage = maxDamage / 100.0;
	if(random_range(0, 100) < CRITICAL_CHANCE){
		maxDamage += int(newDamage * CRITICAL_DAMAGE);
	}
	
	return (maxDamage + attackValue);
}
...
void Weapon::onUsedWeapon(Player* player, Item* item, Tile* destTile) const
{
	if(ammoAction == AMMOACTION_REMOVECOUNT){
		...
		if(g_config.getString(ConfigManager::REMOVE_AMMUNATION) == "yes")
		#endif
