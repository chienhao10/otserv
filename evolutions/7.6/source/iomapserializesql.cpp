...
bool IOMapSerializeSQL::saveTile(Database* db, uint32_t tileId, const Tile* tile)
{
	...
	query_insert.setQuery("INSERT INTO `tileitems` (`tileid`, `sid` , `pid` , `type` , `number` , `attributes` ) VALUES ");
...
bool IOMapSerializeSQL::loadHouseInfo(Map* map, const std::string& identifier)
{
	...
	for(uint32_t i = 0; i < result.getNumRows(); ++i){
		...
		if(house){
			std::string owner = result.getDataString("owner", i);
	...
	for(HouseMap::iterator it = Houses::getInstance().getHouseBegin(); it != Houses::getInstance().getHouseEnd(); ++it){
		query.reset();
		House* house = it->second;
		if(house->getHouseOwner() != "" && house->getHouseId() != 0){
