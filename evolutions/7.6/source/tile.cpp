				...
				if(iiType.blockSolid){
					if(item->isPickupable()){
						...
						if(iitem->getID() == ITEM_DUSTBIN)
							continue;
						else if(iiType.isWater())
							continue;
						else if(iiType.isLava())
							continue;
						else if(iiType.isSwamp())
							continue;
...
bool Tile::__removeThing(Thing* thing, uint32_t count)
{
	Creature* creature = thing->getCreature();
	if(creature){
...
long Tile::clean()
{
	...
	for (int i = 0; i < getThingCount(); i++){
		Item* item = dynamic_cast<Item*>(__getThing(i));
	...
	for(std::list<Item*>::iterator iter = trash.begin(); iter != trash.end(); ++iter){
		...
		if(container){
			...
			while(piter != Player::listPlayer.list.end()){
				Player* player = piter->second;				
				ContainerVector::const_iterator citer = player->getContainers();

				while(citer != player->getEndContainer()){
		...
		if(item->getUniqueId() == 0 && __removeThing(item, item->getItemCount()))
			(*iter)->releaseThing2();
	}
