...
bool Spawn::findPlayer(const Position& pos)
{
	...
	for(it = list.begin(); it != list.end(); ++it) {
		if((tmpPlayer = (*it)->getPlayer()) && tmpPlayer->getAccessLevel() < ACCESS_PROTECT){
			return true;
		}
	}
...
bool Spawn::spawnMonster(uint32_t spawnId, MonsterType* mType, const Position& pos, Direction dir)
{
	...
	if(g_game.getGameState() == GAME_STATE_STARTUP){
		...
		if(!g_game.internalPlaceCreature(pos, monster, true)){
			delete monster;
			return false;
		}
	}
	else{
		if(!g_game.placeCreature(pos, monster, true)){
			delete monster;
			return false;
		}
	}
